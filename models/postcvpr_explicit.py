...
class Config:
    ...
    n_nodefeatures: int = 27            # number of dimensions in node feature vectors (don't chenge), originally 24
    ...
    def add_node_features(self, sample):
        ...
        for k in ['cloth', 'obstacle']:
            ...
            if 'bending_coeff' in sample[k]:
                ...
                explicit_acc = sample[k].explicit_acc
                ...
                explicit_acc = torch.zeros_like(velocity).to(device)

            node_features = torch.cat(
                [velocity, explicit_acc, vertex_type_emb, vertex_level_emb, normals, timestep, v_mass, bending_coeff, lame_mu,
                 lame_lambda], dim=-1)
