...
class Config:
    ...
    sim_data_dir: Optional[str] = None
...
class VertexBuilder:
    ...
    @staticmethod
    def build(sequence_dict: dict, f_make, idx_start: int, idx_end: int = None, garment_name: str = None, fname: str = None, sample=None) -> np.ndarray:
        """
        ...
        verts = f_make(sequence_dict['body_pose'][idx_start: idx_end],
                       ...
                       betas, garment_name=garment_name, fname=fname, idx_start=idx_start, idx_end=idx_end, sample=sample)
...
class GarmentBuilder:
    ...
    def make_cloth_verts(self, body_pose: np.ndarray, global_orient: np.ndarray, transl: np.ndarray, betas: np.ndarray,
        ...
        if garment_name in ['longsleeve', 'tshirt', 'tshirt_unzipped', 'dress']:
            data_path = os.path.join(self.mcfg.sim_data_dir, f"{garment_name}_{fname}.h5")
            displacement = []
            with h5py.File(data_path, 'r') as f:
                for i in range(idx_start, idx_end):
                    displacement.append(f[f'displacement_{i}'][:])
                idx = f[f'idx_{idx_start}'][:]
            ...
            vertices = body_vertices[:, idx] + displacement
    ...
    def build(self, sample: HeteroData, sequence_dict: dict, idx: int, garment_name: str, fname: str) -> HeteroData:
        """
        ...
        sample = self.vertex_builder.add_verts(sample, sequence_dict, idx, self.make_cloth_verts, "cloth",
                                               garment_name=garment_name, fname=fname)
