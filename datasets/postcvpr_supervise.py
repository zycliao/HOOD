...
class Config:
    ...
    noise_scale: float = 0  # Noise scale for the garment vertices (not used in validation)
    ...
    betas_scale: float = 0  # Scale for the beta parameters (not used if random_betas is False)
    ...
    garment_name: str = 'all'  # Garment name to load (used in training)
...
def create_loader(mcfg: Config):
    ...
    if mcfg.garment_name != 'all':
        for k in list(garments_dict.keys()):
            if k != mcfg.garment_name:
                garments_dict.pop(k)
...
class VertexBuilder:
    ...
    def build(sequence_dict: dict, f_make, idx_start: int, idx_end: int = None, garment_name: str = None) -> np.ndarray:
        ...
        verts = f_make(sequence_dict['body_pose'][idx_start: idx_end],
                       ...
                       betas, garment_name=garment_name, idx_start=idx_start, idx_end=idx_end)
    ...
    def add_verts(self, sample: HeteroData, sequence_dict: dict, idx: int, f_make, object_key: str,
        ...
        # if 'garment_name' in kwargs:
        #     writePC2('test_cloth.pc2', all_vertices)
        # else:
        #     writePC2('test_body.pc2', all_vertices)
...
class GarmentBuilder:
    ...
    def __init__(self, mcfg: Config, garments_dict: dict, garment_smpl_model_dict: Dict[str, GarmentSMPL]):
        ...
        self.gt_verts = read_pc2(self.mcfg.gt_seq_path).astype(np.float32)
    ...
    def make_cloth_verts(self, body_pose: np.ndarray, global_orient: np.ndarray, transl: np.ndarray, betas: np.ndarray,
                         garment_name: str, idx_start, idx_end) -> np.ndarray:
        """
        ...
        # Zhou: now we only need to load the ground truth
        vertices = self.gt_verts[idx_start:idx_end]
        if self.mcfg.keep_length and self.mcfg.wholeseq:
            vertices = np.concatenate([vertices[0:1], vertices[0:1], vertices], axis=0)
        # from utils.mesh_io import writePC2
        # writePC2('test_gt.pc2', vertices.numpy())
        ...
        # writePC2('test_smpl.pc2', vertices)
    ...
    def make_shaped_restpos(self, sequence_dict: dict, garment_name: str) -> np.ndarray:
        ...
        verts = self.make_cloth_verts(body_pose,
                                      global_orient,
                                      transl,
                                      sequence_dict['betas'], garment_name=garment_name, idx_start=0, idx_end=1)
        return verts
